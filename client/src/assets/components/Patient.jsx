...
const Patient = ({ _id, username, name, surname, userType, userStatus, createdAt, }) => {
  ...
  return (
    <Wrapper>
      <header>
        {/* ✅ แสดงอักษรแรกของชื่อ */}
        <div className={`main-icon ${searchValues.userStatus === "คนไข้ที่ขาดการทำกายภาพบำบัด" ? "!bg-yellow-600" : "" }`}>{name.charAt(0)}</div>
        <div className="info">
          <h5>{name}</h5>
          <p>{surname}</p>
          <br />
      ...
      <footer>
        <Link
          to={`../edit-patient/${_id}`}
          className={`btn edit-btn ${searchValues.userStatus === "คนไข้ที่ขาดการทำกายภาพบำบัด" ? "!bg-yellow-600" : "" }`}
          style={{
            display: "block",
            marginLeft: "auto",
            marginRight: "auto",
